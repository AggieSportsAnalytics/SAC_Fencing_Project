...
class Import extends NodeBase {
    ...
    setResolution(exportMode, resolution, inlineNamespace = false) {
...
class Chunk$1 {
    ...
    finaliseDynamicImports(stripKnownJsExtensions) {
...
class FileEmitter {
    ...
    finalizeAsset(consumedFile, source, referenceId, output) {
        ...
        output.bundle[fileName] = {
            fileName,
            get isAsset() {
                graph.warnDeprecation('Accessing "isAsset" on files in the bundle is deprecated, please use "type === \'asset\'" instead', true);
                return true;
            },
            source,
            type: 'asset'
...
function getPluginContexts(pluginCache, graph, fileEmitter) {
    ...
    return (plugin, pidx) => {
        ...
        const context = {
            ...
            cache: cacheInstance,
            emitAsset: getDeprecatedContextHandler((name, source) => fileEmitter.emitFile({ type: 'asset', name, source }), 'emitAsset', 'emitFile', plugin.name, true, graph),
            emitChunk: getDeprecatedContextHandler((id, options) => fileEmitter.emitFile({ type: 'chunk', id, name: options && options.name }), 'emitChunk', 'emitFile', plugin.name, true, graph),
            ...
            resolveId: getDeprecatedContextHandler((source, importer) => graph.moduleLoader
                ...
                .then(resolveId => resolveId && resolveId.id), 'resolveId', 'resolve', plugin.name, true, graph),
...
class PluginDriver {
    constructor(graph, userPlugins, pluginCache, basePluginDriver) {
    ...
    constructor(options, watcher) {
        ...
        if (options.treeshake !== false) {
            ...
            if (typeof this.treeshakingOptions.pureExternalModules !== 'undefined') {
                this.warnDeprecation(`The "treeshake.pureExternalModules" option is deprecated. The "treeshake.moduleSideEffects" option should be used instead. "treeshake.pureExternalModules: true" is equivalent to "treeshake.moduleSideEffects: 'no-external'"`, true);
            }
        }
        ...
        return Promise.all([
            ...
            if (this.preserveModules) {
                ...
                for (const chunkModules of inlineDynamicImports
                    ...
                    chunks.push(new Chunk$1(this, chunkModules));
                }
            }
...
const createGetOption = (config, overrides) => (name, defaultValue) => overrides[name] !== undefined
    ? overrides[name]
...
const getExternal = (config, overrides) => {
    ...
    return typeof configExternal === 'function'
        ? (id, ...rest) => configExternal(id, ...rest) || overrides.external.indexOf(id) !== -1
...
function parseInputOptions(config, overrides = { external: [], globals: undefined }, defaultOnWarnHandler = defaultOnWarn) {
...
function parseOutputOptions(config, warn, overrides = {}) {
    const getOption = createGetOption(config, overrides);
    const outputOptions = {
        amd: { ...config.amd, ...overrides.amd },
...
async function rollupInternal(rawInputOptions, watcher) {
    ...
    try {
        ...
        chunks = await graph.build(inputOptions.input, inputOptions.manualChunks, inputOptions.inlineDynamicImports);
    }
    catch (err) {
    ...
    async function generate(outputOptions, isWrite, outputPluginDriver) {
        timeStart('GENERATE', 1);
        if (outputOptions.dynamicImportFunction) {
            graph.warnDeprecation(`The "output.dynamicImportFunction" option is deprecated. Use the "renderDynamicImport" plugin hook instead.`, false);
        }
        ...
        for (const key of Object.keys(outputBundle)) {
            ...
            if (!file.type) {
                graph.warnDeprecation('A plugin is directly adding properties to the bundle object in the "generateBundle" hook. This is deprecated and will be removed in a future Rollup version, please use "this.emitFile" instead.', true);
                file.type = 'asset';
            }
        }
    ...
    const result = {
        ...
        generate: (rawOutputOptions) => {
            ...
            return generate(outputOptions, false, outputPluginDriver).then(result => createOutput(result));
        },
        ...
        write: (rawOutputOptions) => {
            ...
            return generate(outputOptions, true, outputPluginDriver).then(async (bundle) => {
                await Promise.all(Object.keys(bundle).map(chunkId => writeOutputFile(bundle[chunkId], outputOptions)));
...
function normalizeOutputOptions(inputOptions, rawOutputOptions, hasMultipleChunks, outputPluginDriver) {
    const outputOptions = parseOutputOptions(outputPluginDriver.hookReduceArg0Sync('outputOptions', [rawOutputOptions.output || inputOptions.output || rawOutputOptions], (outputOptions, result) => result || outputOptions, pluginContext => {
...
export { parseOutputOptions as a, warnUnknownOptions as b, createCommonjsModule as c, rollupInternal as d, ensureArray as e, fseventsImporter as f, getCjsExportFromNamespace as g, parseInputOptions as p, rollup as r, unwrapExports as u, version as v, watch as w };
